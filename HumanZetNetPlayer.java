...
public class HumanZetNetPlayer extends JPanel
    implements ZetPlayer, MouseListener, ActionListener, KeyListener, Serializable
{
  private final ZetNetGame game;
  ...
  public void setZetGameModel(ZetNetGameModel gameModel)
  {
      this.gameModel = gameModel;
  }
  public HumanZetNetPlayer(ZetNetGame game, ZetNetGameModel gameModel)
  {
    this.game = game;
    this.gameModel = gameModel;
...
//    timer = new Timer(5000, new TimeoutListener(this));
//    timer.start();
  }
  ...
  public void mousePressed(MouseEvent e)
  {
    ...
    if (state != PICKING || cardsPicked >= 3)
      return;
    ZetNetGame.ZetNetTableDisplay d = (ZetNetGame.ZetNetTableDisplay)e.getSource();
    int i = d.getCardLocation(e.getX(), e.getY());
    if (gameModel.pickCard(i))
    {
      cardsPicked++;
      game.connection.send(gameModel);
    }
    if(cardsPicked == 3) 
    {
        if (state == PICKING)
